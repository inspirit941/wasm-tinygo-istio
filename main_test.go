...
func TestHttpHeaders_OnHttpRequestHeaders(t *testing.T) {
	vmTest(t, func(t *testing.T, vm types.VMContext) {
		...
		hs := [][2]string{{ceSpecVersionHeader, "1.0"}, {ceSourceHeader, "//eventsource-1.example.com"}}
		...
		for _, val := range resultHeaders {
			if val[0] == ceSpecVersionHeader {
				require.Equal(t, "1.0", val[1])
				found = true
			}
		}
		...
		require.Contains(t, logs, "[ce-filter] setting cloud event partitionkey: //eventsource-1.example.com")
	})
}
